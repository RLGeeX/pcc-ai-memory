...
**GCP Secret Manager (Terraform Approach)**:

Add to `$HOME/pcc/infra/pcc-devops-infra/terraform/environments/nonprod/secrets.tf`:

```hcl
# Cloudflare Credential Management via Secret Manager
# Module source: pcc-tf-library/modules/secret-manager
# Resources created in: pcc-prj-devops-nonprod
# Purpose: Store Cloudflare API credentials for WireGuard DNS registration

# Sensitive Variables - Store actual values in terraform.tfvars (gitignored)
variable "cloudflare_api_token" {
  description = "Cloudflare API token for DNS management (Zone.DNS edit permission)"
  type        = string
  sensitive   = true
}

variable "cloudflare_zone_id" {
  description = "Cloudflare Zone ID for pcconnect.ai domain"
  type        = string
  sensitive   = true
}

# Module: Cloudflare API Token Secret
# Stores the Cloudflare API token for WireGuard instance DNS registration
# IAM: WireGuard service account granted secretAccessor role
module "cloudflare_api_token_secret" {
  source = "git::ssh://git@github-pcc/PORTCoCONNECT/pcc-tf-library.git//modules/secret-manager?ref=v0.1.0"
  ...
  labels = {
    purpose     = "wireguard-dns"
    environment = "nonprod"
    service     = "wireguard-vpn"
    managed_by  = "terraform"
  }

  rotation_period = null # Disabled - manual rotation via Cloudflare dashboard; no automated rotation mechanism

  # Single-region replication - org policy forbids automatic/global
  replication_policy    = "user-managed"
  replication_locations = ["us-east4"]
}

# Module: Cloudflare Zone ID Secret
# Stores the Cloudflare Zone ID for pcconnect.ai domain
# Static metadata - no rotation needed
module "cloudflare_zone_id_secret" {
  source = "git::ssh://git@github-pcc/PORTCoCONNECT/pcc-tf-library.git//modules/secret-manager?ref=v0.1.0"
  ...
  rotation_period = null # No rotation for static metadata
...
# IAM Binding: Grant WireGuard Service Account Access to Cloudflare API Token
# Allows WireGuard instances to retrieve API token during startup
module "cloudflare_api_token_iam" {
  source = "git::ssh://git@github-pcc/PORTCoCONNECT/pcc-tf-library.git//modules/iam-member?ref=v0.1.0"

  project = var.project_id
  members = ["serviceAccount:wireguard-vpn-sa@pcc-prj-devops-nonprod.iam.gserviceaccount.com"]
  roles   = ["roles/secretmanager.secretAccessor"]

  # This module handles the binding via google_project_iam_member resource
  # Non-authoritative binding - does not affect other secret IAM policies
}

# IAM Binding: Grant WireGuard Service Account Access to Cloudflare Zone ID
# Allows WireGuard instances to retrieve zone ID during startup
module "cloudflare_zone_id_iam" {
  source = "git::ssh://git@github-pcc/PORTCoCONNECT/pcc-tf-library.git//modules/iam-member?ref=v0.1.0"
...
# Outputs for verification and cross-module reference
output "cloudflare_api_token_secret_name" {
  description = "Full secret name for Cloudflare API token"
  value       = module.cloudflare_api_token_secret.secret_name
}

output "cloudflare_zone_id_secret_name" {
  description = "Full secret name for Cloudflare Zone ID"
  value       = module.cloudflare_zone_id_secret.secret_name
}
```

**Add to terraform.tfvars (gitignored)**:
```hcl
# Cloudflare credentials for WireGuard DNS registration
...
**File**: `$HOME/pcc/infra/pcc-devops-infra/terraform/environments/nonprod/wireguard-vpn.tf`
...
**File**: `$HOME/pcc/infra/pcc-devops-infra/terraform/environments/nonprod/startup-script.sh`
...
1. **Cloudflare API Token**:
   ...
   - Stored in GCP Secret Manager (Terraform-managed)
   - Accessed via service account (least privilege)
   - IAM bindings managed via `pcc-tf-library/modules/iam-member`
...
- GCP Secret Manager for credentials (Terraform-managed)
- Service account IAM permissions for Secret Manager (Terraform-managed via `iam-member` module)
- `pcc-tf-library` modules:
  - `secret-manager` (v0.1.0+)
  - `iam-member` (v0.1.0+)

## Related Files

- `$HOME/pcc/infra/pcc-devops-infra/terraform/environments/nonprod/wireguard-vpn.tf`
- `$HOME/pcc/infra/pcc-devops-infra/terraform/environments/nonprod/secrets.tf` (NEW)
- `$HOME/pcc/infra/pcc-devops-infra/terraform/environments/nonprod/startup-script.sh`
- `$HOME/pcc/core/pcc-tf-library/modules/instance-template/` (already supports enable_external_ip)
- `$HOME/pcc/core/pcc-tf-library/modules/secret-manager/` (for Cloudflare credentials)
- `$HOME/pcc/core/pcc-tf-library/modules/iam-member/` (for IAM bindings)
...
- **Terraform Pattern**: Uses established PCC patterns from `pcc-app-shared-infra/terraform/environments/devtest/secrets.tf`
- **IAM Management**: Uses `iam-member` module for non-authoritative bindings (production pattern)
- **Security**: All secrets stored in Secret Manager with user-managed single-region replication per org policy
...
- [PCC Terraform Patterns](file:///home/jfogarty/pcc/infra/pcc-app-shared-infra/terraform/environments/devtest/secrets.tf)
